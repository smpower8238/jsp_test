...
public class DB_Connect {
	...
	public void Create_User(String id, String password) throws SQLException {
		Connect();
	...
	public List<Item> select_board_test(String id) throws SQLException {
		Connect();
		String sql;
		List<Item> list = new ArrayList<Item>();
		if (SaveId.id != null) {
			sql = "select * from test_board where id =? order by num desc";
			psmt = con.prepareStatement(sql);
			psmt.setString(1, id);
		} else {
			sql = "select * from test_board order by num desc";
			psmt = con.prepareStatement(sql);
		...
		while (rs.next()) {
			list.add(new Item(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5)));
	...
	public void Create_board_test(String id, String title, String content) throws SQLException {
		Connect();

		try {
			String sql = "insert into test_board VALUES(SEQ.nextval,?,?,?,SYSDATE)";
	...
	public List<Item> select_boardLookUp_test(int num) throws SQLException {
		Connect();
		String sql;
		List<Item> list = new ArrayList<Item>();
		if (SaveId.id != null){
			sql = "select * from test_board where num =?";
			psmt = con.prepareStatement(sql);
			psmt.setInt(1, num);
		} else {
	...
	public void Delete_User_Test(String id) throws SQLException {
		Connect();
